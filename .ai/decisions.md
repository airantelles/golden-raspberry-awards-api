...
Context7 is configured only in `.codex/config.toml`, using the fixed package version `@upstash/context7-mcp@4.1.1`. If authentication is needed, the server receives it from the local environment; no credential is stored in the repository.
